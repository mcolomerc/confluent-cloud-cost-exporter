...
func (mc JSONMetricCollector) Collect(ch chan<- prometheus.Metric) {
	for _, m := range mc.JSONMetrics {
		...
		case config.ValueScrape:
			...
			if err != nil {
				log.Println("Failed to extract value for metric", "path", m.KeyJSONPath, "err", err, "metric", m.Desc)
				continue
			}
			...
			if floatValue, err := SanitizeValue(value); err == nil {
				...
				log.Println("Failed to convert extracted value to float64", "path", m.KeyJSONPath, "value", value, "err", err, "metric", m.Desc)
				continue
			}
		...
		case config.ObjectScrape:
			...
			if err != nil {
				log.Println("Failed to extract json objects for metric", "err", err, "metric", m.Desc)
				continue
			}
			...
			if err := json.Unmarshal([]byte(values), &jsonData); err == nil {
				for _, data := range jsonData {
					...
					if err != nil {
						log.Println("Failed to marshal data to json", "path", m.ValueJSONPath, "err", err, "metric", m.Desc, "data", data)
						continue
					}
					value, err := extractValue(jdata, m.ValueJSONPath, false)
					if err != nil {
						log.Println("Failed to extract value for metric", "path", m.ValueJSONPath, "err", err, "metric", m.Desc)
						continue
					}
					...
					if floatValue, err := SanitizeValue(value); err == nil {
						...
						log.Println("Failed to convert extracted value to float64", "path", m.ValueJSONPath, "value", value, "err", err, "metric", m.Desc)
						continue
					}
				}
			} else {
				log.Println("Failed to convert extracted objects to json", "err", err, "metric", m.Desc)
				continue
			}
		default:
			log.Println("Unknown scrape config type", "type", m.Type, "metric", m.Desc)
			continue
		}
	}
}
...
func extractValue(data []byte, path string, enableJSONOutput bool) (string, error) {
	...
	if err := json.Unmarshal(data, &jsonData); err != nil {
		log.Println("Failed to unmarshal data to json", "err", err, "data", data)
		return "", err
	}

	if err := j.Parse(path); err != nil {
		log.Println("Failed to parse jsonpath", "err", err, "path", path, "data", data)
		return "", err
	}

	if err := j.Execute(buf, jsonData); err != nil {
		log.Println("Failed to execute jsonpath", "err", err, "path", path, "data", data)
		return "", err
	}
...
func extractLabels(data []byte, paths []string) []string {
	...
	for i, path := range paths {
		if result, err := extractValue(data, path, false); err == nil {
			...
			log.Println("ailed to extract label value", "err", err, "path", path, "data", data)
		}
	}
	return labels
}
...
func timestampMetric(m JSONMetric, data []byte, pm prometheus.Metric) prometheus.Metric {
	...
	if err != nil {
		log.Println("Failed to extract timestamp for metric", "path", m.KeyJSONPath, "err", err, "metric", m.Desc)
		return pm
	}
	epochTime, err := SanitizeIntValue(ts)
	if err != nil {
		log.Println("Failed to parse timestamp for metric", "path", m.KeyJSONPath, "err", err, "metric", m.Desc)
		return pm
	}
