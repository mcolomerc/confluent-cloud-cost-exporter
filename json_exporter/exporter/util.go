...
func (f *JSONFetcher) FetchJSON(endpoint string) ([]byte, error) {
	...
	params := url.Values{}
	// ISSUE - Build params with prometheus target labels
	...
	level.Info(f.logger).Log("msg", "URL", "url", baseURL.String())

	var req *http.Request
	req, err = http.NewRequest(f.method, baseURL.String(), f.body)
	...
	// ISSUE -- https://github.com/prometheus-community/json_exporter/issues/148
	user := os.Getenv("USER")
	pass := os.Getenv("PASS")

	userBearer := user + ":" + pass
	bearer := b64.StdEncoding.EncodeToString([]byte(userBearer))

	if req.Header.Get("Authorization") == "" {
		req.Header.Add("Authorization", "Basic "+bearer)
	} else {
